...
public class SmtpCrawler {
  ...
  @Autowired
  public SmtpCrawler(MeterRegistry meterRegistry, SmtpCrawlService crawlService, SmtpConversationCache cache) {
  ...
  public void process(VisitRequest visitRequest) throws Exception {
    ...
    try {
      ...
      SmtpVisit visit = getSmtpVisit(visitRequest);
      ...
      // Since June 2020, we enable DLQ on SQS, allowing us to not care or to not keep a state/counter/rate-limiter
  ...
  private static @NotNull SmtpVisit getSmtpVisit(VisitRequest visitRequest) {
